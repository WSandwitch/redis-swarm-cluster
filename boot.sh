...
#wait for arbiter
while [ ! "$(redis-cli -h 127.0.0.1 --raw keys '*' >/dev/null && echo 1 || echo 0)" = "1" ]; do
	sleep 1
done
...
if [ ! "$MASTER_NUM" = "$SLOT_NUM" ]; then
	...
	if [ ! "$(redis-cli -h $MASTER_HOST$MASTER_NUM -p $REDIS_PORT cluster nodes | wc -l || echo 0)" = "0" ]; then
